import { expect, test } from '@playwright/test';
import { readE2EEnv } from '../helpers/env';
import {
  adminClient,
  createTestUser,
  deleteTestUser,
  userClient,
} from '../helpers/users';

const env = readE2EEnv();

test.describe('RLS — goals', () => {
  test.skip(
    env === null,
    'Set NEXT_PUBLIC_SUPABASE_URL, NEXT_PUBLIC_SUPABASE_ANON_KEY, and SUPABASE_SERVICE_ROLE_KEY in .env to run RLS tests.',
  );

  test("user B cannot read or modify user A's goals", async () => {
    if (!env) return;
    const userA = await createTestUser(env, 'rls-goals-a');
    const userB = await createTestUser(env, 'rls-goals-b');
    try {
      const admin = adminClient(env);
      const { data: goal, error: insErr } = await admin
        .from('goals')
        .insert({
          user_id: userA.id,
          title: 'Private — A',
          status: 'active',
        })
        .select()
        .single();
      expect(insErr).toBeNull();

      const sb = userClient(env);
      await sb.auth.signInWithPassword({
        email: userB.email,
        password: userB.password,
      });

      const { data: visible } = await sb
        .from('goals')
        .select('id')
        .eq('id', goal!.id);
      expect(visible ?? []).toHaveLength(0);

      const { data: updated } = await sb
        .from('goals')
        .update({ title: 'PWNED' })
        .eq('id', goal!.id)
        .select();
      expect(updated ?? []).toHaveLength(0);

      const { data: deleted } = await sb
        .from('goals')
        .delete()
        .eq('id', goal!.id)
        .select();
      expect(deleted ?? []).toHaveLength(0);

      const { data: stillThere } = await admin
        .from('goals')
        .select('title')
        .eq('id', goal!.id)
        .single();
      expect(stillThere?.title).toBe('Private — A');
    } finally {
      await deleteTestUser(env, userA.id).catch(() => {});
      await deleteTestUser(env, userB.id).catch(() => {});
    }
  });

});
